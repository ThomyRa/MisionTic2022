def serve_client(store_dict, user_dict, temporal_sales_dict):
    ...
    store_stock_updated = {key: value[1] for key,
                           value in zip(report.keys(), report.values())}
    ...
    prices, comic_quants, parcial_prices = store_receipt(
        comics_sold, store_dict)

    # Organizo los datos del recibo en una lista de atributos.
    receipt_attrs = [[comic_quant, price, parcial_price] for comic_quant,
                     price, parcial_price in zip(comic_quants, prices, parcial_prices)]

    # Obtengo los nombres de los artículos.
    products = [name for name in user_dict.keys()]

    # Creo el diccionario con los datos del recibo.
    comics_sold = {product: receipt_attr for product,
                   receipt_attr in zip(products, receipt_attrs)}
    print("El recibo para el usuario es: \n")
    receipt_impression(comics_sold)
    
    # Voy guardando las ventas realizadas para generar posteriormente el reporte de ventas al finalizar el día.
...
def final_report(store_dict, store_stock_updated):
    ...
    """
    for store_key in store_dict.items():
        for stock_key in store_stock_updated.items():
...
def receipt_impression(dictionary):
    ...
    print("".ljust(58, "="))
    print("Comic".ljust(25, ".") + "Cant.".rjust(2) +
          "Precio".rjust(8, ".") + "Precio Total".rjust(20, "."))

    for key, value in dictionary.items():
        print(key.ljust(25, ".") + str(value[0]).rjust(2) + str(
            value[1]).rjust(11, ".") + str(value[2]).rjust(20, "."))
        total_price += value[2]
    print("".ljust(58, "="))
...
def temporal_sales(comics_sold, temporal_sales_dict):
    """Función que va guardando y actualizando las ventas realizadas cada vez que se atiende a un cliente para generar un reporte con las ventas del día después de cerrar la tienda.

    Args:
        comics_sold (dict): Diccionario con los productos vendidos.
        temporal_sales_dict (dict): Diccionario que me va almacenando los productos vendidos.
...
def temporal_sales_impression(dictionary):
    """Imprimo el reporte de ventas del día al cerrar la tienda.

    Args:
        dictionary (dict): Diccionario con cantidades, precios unitarios y precios parciales de las ventas.
    """
    total_price = 0
    print("".ljust(58, "="))
...
def store_stock_alert(store_stock_updated):
    max_len_key = 0
